...
public interface NoticeRepository extends JpaRepository<Notice, Long> {
	...
	@Query(value = "select DISTINCT n.* from ( "
			...
			+ "union all "
			+ "select n.* from notice n where n.status = ?1 and n.appid = ?2 and n.noticeType <>'12'"
			+ "union all "
			+ "select n.* from notice n join noticeSect ns on n.id = ns.noticeId "
			+ "where n.status = ?1 and ns.sectId = ?3 "
			+ "union all "
			+ "select n.* from notice n where n.status = ?1 and n.appid = ?2 and n.openid= ?4 and n.noticeType = '12'"
			+ ") n ", 
			nativeQuery = true)
