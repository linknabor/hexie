...
public interface ServiceOrderRepository extends JpaRepository<ServiceOrder, Long> {
    ...
    @Query(value = "select distinct p.* from ServiceOrder p "
    		...
    		+ "and if(?5!=NULL, p.groupRuleId = ?5, 1=1) "
    		...
    		, countQuery = "select count(distinct p.*) from ServiceOrder p "
    	    		...
    	    		+ "and p.groupOrderId is not null order by p.id desc "
    		, nativeQuery = true)
    List<ServiceOrder> findByUserAndStatusAndTypeV3(long userId, List<Integer> status, int orderType, String productName, Long ruleId, List<Integer> itemStatus, Pageable page);
