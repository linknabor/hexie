...
public class RgroupServiceImpl implements RgroupService {
	...
	@Override
	public List<RgroupOrdersVO> queryMyRgroupOrdersV3(long userId, List<Integer> status, String productName, List<Integer> itemStatus, int currentPage) {
		
		Pageable pageable = PageRequest.of(currentPage, 10);
		List<ServiceOrder> orders = serviceOrderRepository.findByUserAndStatusAndTypeV3(userId, status, 
				ModelConstant.ORDER_TYPE_RGROUP, productName, itemStatus, pageable);
